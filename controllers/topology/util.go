...
// ResolveHostNetwork returns true if the topology resource should run in host network mode.
func ResolveHostNetwork(t *clabernetesapisv1alpha1.Topology) bool {
	if t.Spec.Deployment.HostNetwork == nil {
		return false
	}

	return *t.Spec.Deployment.HostNetwork
}
