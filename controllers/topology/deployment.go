...
func (r *DeploymentReconciler) renderDeploymentContainer(
	...
	if nodeDef, ok := clabernetesConfigs[nodeName].Topology.Nodes[nodeName]; ok {
		...
		// vrnetlab-based Cisco IOL/IOLL2 requires containerlab kind-driver setup (NETMAP/iouyap.ini/nvram),
		...
		if strings.EqualFold(nodeDef.Kind, "cisco_iol") || strings.EqualFold(nodeDef.Kind, "cisco_ioll2") {
			existingMounts := map[string]struct{}{}
			for _, vm := range nosContainer.VolumeMounts {
				existingMounts[strings.TrimSpace(vm.MountPath)] = struct{}{}
			}
...
CFGEOF
	...
	# netlab 26.02+ can generate a containerlab "startup partial config" bundle, which is the
	# canonical native way to inject configuration into IOS/IOL containers. Prefer it when present
	# to avoid applying individual cfglets out of order (and to avoid embedding ifstate.cfg, which
	# ends with an "end" marker and would truncate the remaining config).
	startup_partial=""
	for f in /tmp/skyforge-c9s/*/node_files/${node}/startup.partial.config; do
	  if [ -f "$f" ]; then
	    startup_partial="$f"
	    break
	  fi
	...
	if [ -z "$startup_partial" ]; then
	  echo "[skyforge] ERROR: missing netlab startup.partial.config for ${node} (expected /tmp/skyforge-c9s/*/node_files/${node}/startup.partial.config)" >&2
	  exit 1
	fi
	...
	awk '
	  ...
	  { print }
	' "$startup_partial" >> /vrnetlab/config.txt
...
func (r *DeploymentReconciler) renderDeploymentContainerEnv( //nolint: funlen
	...
	if ResolveNativeMode(owningTopology) {
		...
		// Cisco IOL/IOLL2 in native mode relies on the launcher-side SSH proxy
		// (podIP:22 -> 169.254.100.2:22). Enable it automatically for these kinds
		// so netlab 26.02 startup.partial.config workflows stay SSH-reachable.
		nodeKindRaw, _ := clabernetesConfigs[nodeName].Topology.GetNodeKindType(nodeName)
		nodeKind := strings.ToLower(strings.TrimSpace(nodeKindRaw))
		nodeImageLower := strings.ToLower(strings.TrimSpace(nodeImage))
		if nodeKind == "cisco_iol" ||
			nodeKind == "cisco_ioll2" ||
			nodeKind == "iol" ||
			nodeKind == "ioll2" ||
			strings.Contains(nodeImageLower, "/vrnetlab/cisco_iol") {
			envs = append(
				envs,
				k8scorev1.EnvVar{
					Name:  "SKYFORGE_VRNETLAB_SSH_PROXY",
					Value: clabernetesconstants.True,
				},
			)
		}
